...
gsap.to('#nav', { opacity: 1, y: 0, duration: 0.8, ease: 'power2.out', delay: 0.3 });

const heroTL = gsap.timeline({ delay: 0.5 });
heroTL
  .to('.hero__tag', { opacity: 1, y: 0, duration: 0.5, ease: 'power2.out' })
  .to('.hero__title .word', { opacity: 1, y: 0, duration: 0.6, stagger: 0.12, ease: 'back.out(1.5)' }, '-=0.2')
...
gsap.to('.stethoscope-icon', { y: -12, duration: 2.5, ease: 'sine.inOut', yoyo: true, repeat: -1, delay: 1.5 });
...
ScrollTrigger.create({
  ...
  onEnter() {
    ...
    tl.to('.doctor-card__img-wrap', { opacity: 1, x: 0, duration: 0.8, ease: 'power3.out' })
      .to('.doctor-card__badge', { opacity: 1, scale: 1, duration: 0.5, ease: 'back.out(2)' }, '-=0.3')
      .to('.about__right', { opacity: 1, x: 0, duration: 0.8, ease: 'power3.out' }, '-=0.6')
      .to('.credential', { opacity: 1, x: 0, duration: 0.5, stagger: 0.15, ease: 'power2.out' }, '-=0.4');
  },
});
...
ScrollTrigger.create({
  ...
  onEnter() {
    const tl = gsap.timeline();
    tl.to('.services__header', { opacity: 1, y: 0, duration: 0.6, ease: 'power2.out' })
      .to('.service-card', { opacity: 1, scale: 1, y: 0, duration: 0.55, stagger: { each: 0.1, from: 'start' }, ease: 'back.out(1.3)' }, '-=0.2');
  },
});
...
ScrollTrigger.create({
  ...
  onEnter() {
    ...
    tl.to('#testimonials .section__tag', { opacity: 1, duration: 0.4, ease: 'power2.out' })
      ...
      .to('.testimonial-card', { opacity: 1, y: 0, duration: 0.6, stagger: 0.1, ease: 'power2.out' }, '-=0.2')
      .to('.stats-row', { opacity: 1, y: 0, duration: 0.6, ease: 'power2.out' }, '-=0.2');
    qsa('.stat__num').forEach(el => {
      const target = parseInt(el.dataset.target, 10);
      gsap.to({ val: 0 }, { val: target, duration: 2, ease: 'power2.out', delay: 0.4, onUpdate() { el.textContent = Math.round(this.targets()[0].val).toLocaleString(); } });
    });
  },
});
...
function goToSlide(idx) {
  currentSlide = idx;
  gsap.to(track, { x: -(idx * track.parentElement.offsetWidth), duration: 0.55, ease: 'power2.inOut' });
  dots.forEach((d, i) => d.classList.toggle('dot--active', i === idx));
}

dots.forEach((dot, i) => dot.addEventListener('click', () => goToSlide(i)));
const autoSlide = setInterval(() => goToSlide((currentSlide + 1) % cards.length), 4000);
...
ScrollTrigger.create({
  ...
  onEnter() {
    const tl = gsap.timeline();
    tl.to('.contact__left', { opacity: 1, x: 0, duration: 0.8, ease: 'power3.out' })
      .to('.contact__right', { opacity: 1, x: 0, duration: 0.8, ease: 'power3.out' }, '-=0.5');
  },
});
...
qs('#contactForm').addEventListener('submit', e => {
  ...
  setTimeout(() => { text.textContent = 'Confirm Appointment'; btn.style.background = ''; }, 3000);
});
...
function setActive(id) {
  navLinks.forEach(a => { a.style.color = a.getAttribute('href') === '#' + id ? 'var(--teal)' : ''; });
}
